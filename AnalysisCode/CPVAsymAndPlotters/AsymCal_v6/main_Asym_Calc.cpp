...
int main(int argc, char **argv)
{
   ...
   for (int iopt = 0; iopt<argc; ++iopt)
   {
      ...
      if(strcmp(argch,"-studyName")==0) // Study Name //
      {
         studyName = argv[iopt+1];
         iopt = iopt+1;
      }

      if(strcmp(argch,"-systName")==0) // Systematic Name //
      {
      ...
      if(strcmp(argch,"-outputO3version")==0) // Output File //
      {
         outputO3version = argv[iopt+1];
         iopt = iopt+1;
      }
   ...
   " studyName : " << studyName << "\n" <<
   " outputDir : " << outputDir << "\n" <<
   " outputO3version : " << outputO3version << "\n" <<
   " object : " << object << "\n" <<
   ...
   if (systName == "none" || systName == "None"){
       asymOut = Form("./output/%s/%s_%s_%s_%s.txt",outputDir.c_str(),Chan.Data(),object.c_str(),studyName.c_str(),outputO3version.c_str());
       xsecOut = Form("./output/%s/%s_%s_%s_%s_diff.txt",outputDir.c_str(),Chan.Data(),object.c_str(),studyName.c_str(),outputO3version.c_str());
   }
   else {
