...
/*            cout  << " Info_EventNumber : " << Info_EventNumber << " step 5 evt_weight_ : " << v_SystEvt[1] 
                  ...
                  if(dR_bTagJet_Lep < dR_bTagJet_AnLep) //bTagJet is close to Lep, it means bTagJet has minus charge, that is AnbJet
                  {
                     ...
                     //dR distribution btw pair_bJet and Lep, AnLep: after pairing
                     //FillHisto(h_dR_pair_bJet_AnLep, dR_pair_bJet_AnLep);
                     //FillHisto(h_dR_pair_AnbJet_Lep, dR_pair_AnbJet_Lep);
                     //FillHisto(h_dR_bJet_AnbJet,dR_pair_bJet_AnLep,dR_pair_AnbJet_Lep);
                  }
                  else //bTagJet is close to AnLep
                  {
                     ...
                     dR_pair_AnbJet_Lep = pair_AnbJet->DeltaR(*((TLorentzVector*)pair_Lep));
                     // after pairing
                     //FillHisto(h_dR_pair_bJet_AnLep, dR_pair_bJet_AnLep);      //dR distribution b quark & lepton
                     //FillHisto(h_dR_pair_AnbJet_Lep, dR_pair_AnbJet_Lep);  //dR distribution b-bar quark & anti-lepton
                     //FillHisto(h_dR_bJet_AnbJet,dR_pair_bJet_AnLep,dR_pair_AnbJet_Lep);
                  }
                  ...
                  if(dR_pair_bJet_AnLep < 3 && dR_pair_AnbJet_Lep < 3) // bJet-Lep, AnbJet-AnLep간의 거리가 3이하일때
                  {
                     FillHisto(h_dR3_pair_bJet_AnLep    ,dR_pair_bJet_AnLep   ,evt_weight_);
                     FillHisto(h_dR3_pair_AnbJet_Lep  ,dR_pair_AnbJet_Lep     ,evt_weight_);
                     FillHisto(h_dR3_pair_bJet_AnbJet ,dR_pair_bJet_AnLep  ,dR_pair_AnbJet_Lep  ,evt_weight_);
                     FillHisto(h_Pair_bJetpt     ,pair_bJet->Pt()    ,evt_weight_);
                     FillHisto(h_Pair_bJeteta    ,pair_bJet->Eta()   ,evt_weight_);
                     FillHisto(h_Pair_bJetphi    ,pair_bJet->Phi()   ,evt_weight_);
                     FillHisto(h_Pair_AnbJetpt   ,pair_AnbJet->Pt()  ,evt_weight_);
                     FillHisto(h_Pair_AnbJeteta  ,pair_AnbJet->Eta() ,evt_weight_);
                     FillHisto(h_Pair_AnbJetphi  ,pair_AnbJet->Phi() ,evt_weight_);
                     FillHisto(h_pair_Leppt      ,pair_Lep->Pt()     ,evt_weight_);
                     FillHisto(h_pair_Lepeta     ,pair_Lep->Eta()    ,evt_weight_);
                     FillHisto(h_pair_Lepphi     ,pair_Lep->Phi()    ,evt_weight_);
                     FillHisto(h_pair_AnLeppt    ,pair_AnLep->Pt()   ,evt_weight_);
                     FillHisto(h_pair_AnLepeta   ,pair_AnLep->Eta()  ,evt_weight_);
                     FillHisto(h_pair_AnLepphi   ,pair_AnLep->Phi()  ,evt_weight_);
                     ...
                     if (n_cpO3 > 0.0) {FillHisto(h_CPO3_Pos,n_cpO3,evt_weight_);}
                     else {FillHisto(h_CPO3_Neg,n_cpO3,evt_weight_);}
                     FillHisto(h_CPO3_bfReco,n_cpO3,evt_weight_);
                     FillHisto(h_CPO3_bfReco_ReRange,n_cpO3,evt_weight_);
                  }
                  ///// generator level /////
                  //SetGenLepAnLep();
                  int mom1_bidx = -99;
...
void ssb_analysis::DeclareHistos()
{
   ...
   // after pairing: if dR is under 3
   h_dR3_pair_bJet_AnLep     = new TH1D("_h_dR3_pair_bJet_AnLep_"     , "delta R btw pair_bJet & Lep"           , 100, 0, 10); h_dR3_pair_bJet_AnLep->Sumw2();
   h_dR3_pair_AnbJet_Lep   = new TH1D("_h_dR3_pair_AnbJet_Lep_"   , "delta R btw pair_bJet & AnLep"         , 100, 0, 10); h_dR3_pair_AnbJet_Lep->Sumw2();
   h_dR3_pair_bJet_AnbJet  = new TH2D("_h_dR3_pair_bJet_AnbJet_"  , "delta R value scatter: (pair_bJet&pair_Lep) & (pair_AnbJet&AnLep)", 100, 0, 10, 100, 0, 10); h_dR3_pair_bJet_AnbJet->Sumw2();

   // kinematics of bJet, AnbJet, Lep, AnLep after pairing
   h_Pair_bJetpt     = new TH1D(Form("_h_Pair_bJetpt_")     ,Form("b tagging jet(bJet) pT after pairing")      ,1000   ,0        ,1000   ); h_Pair_bJetpt->Sumw2();
   h_Pair_bJeteta    = new TH1D(Form("_h_Pair_bJeteta_")    ,Form("b tagging jet(bJet) eta after pairing")     ,50     ,-2.5     ,2.5    ); h_Pair_bJeteta->Sumw2();
   h_Pair_bJetphi    = new TH1D(Form("_h_Pair_bJetphi_")    ,Form("b tagging jet(bJet) phi after pairing")     ,24     ,-1*pi    ,1*pi   ); h_Pair_bJetphi->Sumw2();
   h_Pair_AnbJetpt   = new TH1D(Form("_h_Pair_AnbJetpt_")   ,Form("b tagging jet pT (AnbJet after pairing)")   ,1000   ,0        ,1000   ); h_Pair_AnbJetpt->Sumw2();
   h_Pair_AnbJeteta  = new TH1D(Form("_h_Pair_AnbJeteta_")  ,Form("b tagging jet eta (AnbJet after pairing)")  ,50     ,-2.5     ,2.5    ); h_Pair_AnbJeteta->Sumw2();
   h_Pair_AnbJetphi  = new TH1D(Form("_h_Pair_AnbJetphi_")  ,Form("b tagging jet phi (AnbJet after pairing)")  ,24     ,-1*pi    ,1*pi   ); h_Pair_AnbJetphi->Sumw2();
   h_pair_Leppt      = new TH1D(Form("_h_pair_Leppt_")      ,Form("Lep pT after pairing")                      ,1000   ,0        ,1000   ); h_pair_Leppt->Sumw2();
   h_pair_Lepeta     = new TH1D(Form("_h_pair_Lepeta_")     ,Form("Lep eta after pairing")                     ,50     ,-2.5     ,2.5    ); h_pair_Lepeta->Sumw2();
   h_pair_Lepphi     = new TH1D(Form("_h_pair_Lepphi_")     ,Form("Lep phi after pairing")                     ,24     ,-1*pi    ,1*pi   ); h_pair_Lepphi->Sumw2();
   h_pair_AnLeppt    = new TH1D(Form("_h_pair_AnLeppt_")    ,Form("AnLep pT (AnbJet after pairing)")           ,1000   ,0        ,1000   ); h_pair_AnLeppt->Sumw2();
   h_pair_AnLepeta   = new TH1D(Form("_h_pair_AnLepeta_")   ,Form("AnLep eta (AnbJet after pairing)")          ,50     ,-2.5     ,2.5    ); h_pair_AnLepeta->Sumw2();
   h_pair_AnLepphi   = new TH1D(Form("_h_pair_AnLepphi_")   ,Form("AnLep phi (AnbJet after pairing)")          ,24     ,-1*pi    ,1*pi   ); h_pair_AnLepphi->Sumw2();
   // O3 before top reconstruction
   h_CPO3_bfReco           = new TH1D(Form("_h_CPO3_bfReco_")           , Form(""), 200   , -10    , 10);   h_CPO3_bfReco->Sumw2();
   h_CPO3_bfReco_ReRange   = new TH1D(Form("_h_CPO3_bfReco_ReRange_")   , Form(""), 40    , -2     , 2);    h_CPO3_bfReco_ReRange->Sumw2();
   h_CPO3_Pos              = new TH1D(Form("_h_CPO3_Pos_")              , Form(""), 200   , -10    , 10);   h_CPO3_Pos->Sumw2();
   h_CPO3_Neg              = new TH1D(Form("_h_CPO3_Neg_")              , Form(""), 200   , -10    , 10);   h_CPO3_Neg->Sumw2();
