...
void valid_ratio()
{
    void ForData(TH1F *h_Single,TH1F *h_Double,TH1F *h_CPV, string list, int idx, string dir);

    std::string dir = "./valid_v3_data";
    gSystem->mkdir(Form("%s",dir.c_str()), kTRUE);

    std::string totalEventCountFilename = dir + "/TotalEventCount_v3.txt";
    ...
    for(int istep = 1; istep <= 8; istep++) // step 1 ~ step 8(top reconstruction)
    {
        ...
        for(int sampleIndex = 0; sampleIndex < samplelist.size(); sampleIndex++)
        {
            for(int ihist=0; ihist<hlist.size(); ihist++)
            {
                ...
                {
                    TFile *fileRepro_Single = new TFile(Form("../output/bfTopReco_v3_dR3cut/Data/Data_SingleMuon.root"), "READ");
                    TFile *fileRepro_Double = new TFile(Form("../output/bfTopReco_v3_dR3cut/Data/Data_DoubleMuon.root"), "READ");
                    ...
                    ForData(hRepro_Single,hRepro_Double,hCPV,samplelist[sampleIndex],istep,dir);
                ...
                TFile *fileRepro = new TFile(Form("../output/bfTopReco_v3_dR3cut/%s/%s/%s_All.root",samplelist[sampleIndex].c_str(), samplelist[sampleIndex].c_str(), samplelist[sampleIndex].c_str()), "READ");
                ...
                if(hRepro && hCPV)
                {
                    ...
                    c1->Print(Form("%s/Valid_%s_%s_step%d.pdf",subdir.c_str(),hlist[ihist].c_str(),samplelist[sampleIndex].c_str(),istep));
...
void ForData(TH1F *h_Single,TH1F *h_Double,TH1F *h_CPV, string list, int idx, string dir)
{
    ...
    TLegend *legendSingle = new TLegend(0.67, 0.6, 0.9, 0.75);
    ...
    TLegend *legendDouble = new TLegend(0.67, 0.45, 0.9, 0.6);
